...
def update_player_statistics(input_path, output_file):
    ...
    var_statree = get_var_statree(statree)
...
def animate_items_given_to_self(base_path, output_file="items_given_to_self.mp4"):
    ...
    for i, iteration_folder in enumerate(sorted(os.listdir(base_path))):
        iteration_path = os.path.join(base_path, iteration_folder, "alice", "statistics")
...
def animate_items_given_to_self_by_round(base_path, output_file="items_given_to_self_by_round.mp4", pause_frames=5):
    """
    Generates an animated histogram of items given to self over iterations from multiple JSON files,
                ...
                if filename.endswith('.json'):
                    with open(os.path.join(iteration_path, filename), 'r') as file:
                        ...
                        for round_key, round_data in data.items():
                            ...
                            if not isinstance(items_given, int) or (items_given < 0 or items_given > 10): items_given = 0
    ...
    fig.subplots_adjust(hspace=0.4, wspace=0.4)  # Add more space between rows and columns

    def update(frame):
        actual_frame = frame // pause_frames  # Determine the actual frame based on pause_frames
