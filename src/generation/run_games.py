...
def run_batched_matches(
    matches, models, export_path, nb_parallel_matches, seed_offset=0
):
    """
    ...
    while active_matches or pending_matches:
        ...
        policy_outputs = process_policy_inputs(
            models, policy_inputs, seed_offset=seed_offset
        )
        ...
        for match_id, match_data in active_matches.items():
            ...
            if action_required_agents == ready_agents:
                ...
                if done:
                    ...
                    agent_infos = [
                        agent.get_log_info() for agent in match_data["agents"].values()
                    ]
        ...
        for match_id in completed_matches:
            ...
            if pending_matches:
                ...
                active_matches[new_match_id] = {
                    ...
                    "pending_actions": {},
                    "policy_outputs": {
                        agent_id: None for agent_id in new_match["agents"]
                    },
