...
def run_matches(
    ...
    for match in parallel_matches:
        for player in match["players"].values():
            if player.llm_and_adapter_id not in llm_and_adapter_ids:
                llm_and_adapter_ids.append(player.llm_and_adapter_id)
    prompt_batches = {llm_and_adapter_id: [] for llm_and_adapter_id in llm_and_adapter_ids}
    response_batches = {llm_and_adapter_id: [] for llm_and_adapter_id in llm_and_adapter_ids}
    ...
    while parallel_matches or all_matches:
        ...
        for match in parallel_matches:
            ...
            prompt_batches[current_player.llm_and_adapter_id].append(
                current_player.get_chat_history()  # No deep copy needed here
            )

        # Process prompts for each model
        for llm_and_adapter_id in llm_and_adapter_ids:
            model_name = llm_and_adapter_id.split("/")[0]
            adapter_name = llm_and_adapter_id.split("/")[1]
            model = models[model_name]
            if prompt_batches[llm_and_adapter_id]:
                if hasattr(model, 'adapters'):
                    model.prepare_adapter_eval(adapter_name)
                response_batches[llm_and_adapter_id] = model.prompt(prompt_batches[llm_and_adapter_id])
            prompt_batches[llm_and_adapter_id] = []

        # Execute player moves based on responses
        for match in parallel_matches[:]:
            match["game_state"] = match["game"].get_state()
            current_player = match["players"][match["game"].get_current_player()]
            response = response_batches[current_player.llm_and_adapter_id].pop(0)
