...
class ClassicAgent(NegotiationAgent):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        # TODO: take inspiration from DOND prompt
        self.intro_prompt = (
            "Classic negotiation over coins. You are {agent_name}. Your per-coin value is revealed, "
            "the other agent may disclose theirs via chat. 10 coins to split."
        )
        self.new_round_prompt = (
            "Round {round_nb}. Your per-coin value: {value}. There are {quantities[coins]} coins."
        )
        self.send_split_prompt = (
            "Respond with <coins_to_self>x</coins_to_self> where x is an integer in [0, {quantities[coins]}]."
        )
